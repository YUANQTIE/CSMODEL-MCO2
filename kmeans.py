...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            for j in range(len(self.centroids)):
                distances[j] = self.get_euclidean_distance(
                    sliced_data, self.centroids.iloc[j]
                )
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        squared_diff = (point1.astype(float) - point2.astype(float)) ** 2
 
        if isinstance(point1, pd.DataFrame):
            # Sum across columns (axis=1) so we get one distance per row.
    ...
    def train(self, data, iters):
        ...
        while i < iters and not flag_groups and not flag_centroids:
            ...
            flag_centroids = np.allclose(
                centroids.values.astype(float),
                self.centroids.values.astype(float)
            )
